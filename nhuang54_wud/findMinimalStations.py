...
class findMinimalStations(dml.Algorithm):
    ...
    # Fix these
    reads = ['nhuang54_wud.boston_bikes']
    writes = ['nhuang54_wud.minimalStations']
    ...
    def execute(trial = False):
        ...
        # Get the data
        bikePaths = repo.nhuang54_wud.boston_bikes.find()
        ...
        for k,v in solution.items():
          repo['nhuang54_wud.minimalStations'].insert_one({k:str(v)})
        repo['nhuang54_wud.minimalStations'].metadata({'complete':True})
        print(repo['nhuang54_wud.minimalStations'].metadata())
