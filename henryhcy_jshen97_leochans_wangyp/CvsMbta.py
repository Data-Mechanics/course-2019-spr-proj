...
class CvsMbta(dml.Algorithm):
    ...
    reads = ['henryhcy_jshen97_leochans_wangyp.cvs',
             'henryhcy_jshen97_leochans_wangyp.mbtaStops']
    writes = ['henryhcy_jshen97_leochans_wangyp.cvsMBTA']
    ...
    def execute(trial=False):
        ...
        for document in repo.henryhcy_jshen97_leochans_wangyp.cvs.find():
            if 'results' in document.keys():
                for item in document['results']:
                    ...
                    repo['henryhcy_jshen97_leochans_wangyp.cvsMBTA'].insert_one(d)
            else:
                continue

        # pick those MBTAs that are within 15 km of Boston and insert
        for document in repo.henryhcy_jshen97_leochans_wangyp.mbtaStops.find():
        ...
        repo['henryhcy_jshen97_leochans_wangyp.cvsMBTA'].metadata({'complete': True})
        print(repo['henryhcy_jshen97_leochans_wangyp.cvsMBTA'].metadata())



        # check structure
        #for document in repo.henryhcy_jshen97_leochans_wangyp.cvsMBTA.find():
    ...
    def provenance(doc=prov.model.ProvDocument(), start_time=None, end_time=None):
        ...
        this_script = doc.agent('alg:henryhcy_jshen97_leochans_wangyp#cvsMBTA',
        ...
        cvsMBTA = doc.entity('dat:henryhcy_jshen97_leochans_wangyp#cvsMBTA', {prov.model.PROV_LABEL: 'Combine CVS mbtaStops', prov.model.PROV_TYPE: 'ont:DataSet'})
